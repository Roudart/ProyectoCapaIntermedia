...
$CursoCalificados = $ConexionCursos->MejorCalificados();
...
<body>
    ...
    <h1 class="text-center">¡Los mejor calificados!</h1>
    <div class="container mt-5">
        <div class="row">
            <div class="col">
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="list-1" role="tabpanel" aria-labelledby="list-1-list">
                        <?php if (isset($CursoCalificados)) {
                            $sizeCursos = sizeof($CursoCalificados);
                            for ($i = 0; $i < $sizeCursos; $i++) {
                                if ($CursoCalificados[$i]["Promedio"] == null) $CursoCalificados[$i]["Promedio"] = '0.0';
                                echo
                                '<form action="curso.php" method="get">
                                <div class="row shadow-sm rounded mb-5 zoom" id="TarjetaCurso">
                                <div class="col-2">
                                    <img src="';
                                if ($CursoCalificados[$i]["ImagenURL"] !== null) {
                                    echo $CursoCalificados[$i]["ImagenURL"];
                                } else {
                                ...
                                <div class="col-10">
                                    <h1>' . $CursoCalificados[$i]["Nombre"] . '</h1>
                                    <p>' . $CursoCalificados[$i]["Descripción"] . '</p>
                                    <input type="text" id="CursoId" name="CursoId" value="' . $CursoCalificados[$i]["IdCurso"] . '"  hidden>
                                    <button type="submit" id ="LinkCurso' . $CursoCalificados[$i]["IdCurso"] . '" class="btn btn-secondary">Ver mas...</button>
                                    <span class="w3-badge w3-large w3-yellow w3-padding">' . $CursoCalificados[$i]["Promedio"] . '</span>
                                </div>
                            </div>
                            </form>
                            ';
